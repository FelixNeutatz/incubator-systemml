...
public class CumulativeOffsetFLInstruction extends BinaryFLInstruction 
{
	...
	public CumulativeOffsetFLInstruction(Operator op, CPOperand in1, CPOperand in2, CPOperand out, double init, String opcode, String istr)
	{
		super(op, in1, in2, out, opcode, istr);
		_fltype = FLINSTRUCTION_TYPE.CumsumOffset;
	 ...
	 */
	public static CumulativeOffsetFLInstruction parseInstruction (String str ) 
		throws DMLRuntimeException 
	{
		...
		return new CumulativeOffsetFLInstruction(null, in1, in2, out, init, opcode, str);
	}
		...
		throws DMLRuntimeException
	{
		...
		MatrixCharacteristics mc = flec.getMatrixCharacteristics(input2.getName());
		long rlen = mc.getRows();
		int brlen = mc.getRowsPerBlock();
		
		//get inputs
		DataSet<Tuple2<MatrixIndexes,MatrixBlock>> inData = flec.getBinaryBlockDataSetHandleForVariable( input1.getName() );
		DataSet<Tuple2<MatrixIndexes,MatrixBlock>> inAgg = flec.getBinaryBlockDataSetHandleForVariable( input2.getName() );
		
		//prepare aggregates (cumsplit of offsets)
		inAgg = inAgg.flatMap(new DataSetCumSplitFunction(_initValue, rlen, brlen));
		
		//execute cumulative offset (apply cumulative op w/ offsets)
		DataSet<Tuple2<MatrixIndexes,MatrixBlock>> out = 
				inData.join( inAgg ).where(0).equalTo(0)
				      .with(new DataSetCumOffsetFunction(_uop, _bop));
		
		updateUnaryOutputMatrixCharacteristics(flec);
	 ...
	 * 
	 */
	private static class DataSetCumSplitFunction implements FlatMapFunction<Tuple2<MatrixIndexes, MatrixBlock>, Tuple2<MatrixIndexes, MatrixBlock>> 
	{
		...
		public DataSetCumSplitFunction( double initValue, long rlen, int brlen )
		{
			...
			throws Exception 
		{
			...
			if( firstBlk ) { 
				...
				out.collect(new Tuple2<MatrixIndexes,MatrixBlock>(tmpix, tmpblk));
			}	
	 ...
	 * 
	 */
	private static class DataSetCumOffsetFunction implements JoinFunction<Tuple2<MatrixIndexes,MatrixBlock>, Tuple2<MatrixIndexes, MatrixBlock>, Tuple2<MatrixIndexes, MatrixBlock>> 
	{
		...
		public DataSetCumOffsetFunction(UnaryOperator uop, BinaryOperator bop)
		{
			_uop = uop;
			_bop = bop;
		}
			...
			throws Exception 
		{
			...
			MatrixBlock dblkIn = a.f1; //original data 
			MatrixBlock oblkIn = b.f1; //offset row vector
